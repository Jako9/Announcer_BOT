...
function printServer(){
    ...
    foreach($servers as $server) {
        ...
        if(isset($_POST['reset-server-settings-'. $i])){
            ...
            $pObj->manageRolle = json_encode($pObj->manageRolle);
            $pObj->whitelist = json_encode($pObj->whitelist);
            $pObj->standartRole = json_encode($pObj->standartRole);
            $pObj->channelReact = json_encode($pObj->channelReact);
            $pObj->lockable = json_encode($pObj->lockable);
...
?>
    ...
    <div id="server" class="d-board-card">
        <div class="container-fluid" >
        <div class="card">
            <h5 class="card-header" id="server-header">Server <?php getServerCountFromDatabase() ?></h5>
    ...
    <div id="vips" class="d-board-card">
        <div class="container-fluid">
        <div class="card">
            <h5 class="card-header">VIPs <?php getVIPCountFromDatabase() ?></h5>
            <div class="card-body">
            <div class="card-text server-body">
            <div class="accordion" id="accordionExample">
