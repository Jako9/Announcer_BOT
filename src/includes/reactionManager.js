...
async function removeForeignReactions(message, reaction){
  ...
  for(let i = 0; i < reactions.length; i++){
    ...
    if(user == undefined) {
      reactions[i].remove().catch(err => {logManager.writeErrorLog(err.stack);});
      if(reaction && reaction.emoji.id == reactions[i].emoji.id){
        removed = true
      }
    }
  }
...
module.exports = {
  ...
  addReactor: function(message){
    ...
    if(serverManager.getChannelReact(message.guild) == null){
      ...
      message.reply('You must select a channel first.').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(!serverManager.getReactionMessage(id)){
      ...
      message.reply('There is no pinned message in this channel. If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(param.length != 2){
      ...
      message.reply('The reaction could not be added. (incorrect Syntax). If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    //Ein custom Emoji
    ...
    //Ein StandartEmoji
      ...
      message.reply('The reaction could not be added. . If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    else{
      ...
      message.reply('The reaction could not be added. (No emoji found). If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    try{
      ...
      message.reply('The reaction has been added successfully.').catch(err => {logManager.writeErrorLog(err.stack);});
    }
    catch(e){
      //Reaktion ungültig
      logManager.writeDebugLog(message.guild.name + ": <span style='color:#c72222;'>FEHLER</span>: Die Rolle der Reaktion konnte nicht vergeben werden (Keine korrekte Syntax).");
  ...
  setupListener: function(message, client, prefix, instructions){
    ...
    if(message.content.split(' ').length < 2 || message.mentions.channels.size != 1){
      ...
      message.reply('Incorrect usage of \'' + prefix +  instructions[20][0].name + '\', type \'' + prefix +  instructions[8][0].name + '\' for the correct syntax. If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(!channel) {
      ...
      message.reply('The channel could not be found. If you need help with the setup, refer to our website @setup').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    channel.messages.fetch();
    if(channel.messages.cache.find(message => message.pinned) == null){
      message.reply("I didn\'t find any pinned messages in this channel. Be sure to first pin the message you want to be reacted on.").catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    message.reply('Setup successful.').catch(err => {logManager.writeErrorLog(err.stack);});
  },

  showChannelReact: function(message){
    if(serverManager.getChannelReact(message.guild) == null){
      message.reply("There is no reaction channel active.").catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    message.reply(serverManager.getChannelReact(message.guild).name).catch(err => {logManager.writeErrorLog(err.stack);});
  },

  removeChannelReact: function(message){
    if(serverManager.getChannelReact(message.guild) == null){
      message.reply("There is no reaction channel setup that can be removed.").catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    serverManager.setChannelReact(message.guild.id, null);
    message.reply("The reaction channel has been removed.").catch(err => {logManager.writeErrorLog(err.stack);});
  }
}
