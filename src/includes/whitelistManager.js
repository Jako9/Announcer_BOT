...
module.exports = {
  ...
  show: function(message){
    ...
    if(channels.length == 0){
      message.reply("The whitelist is empty. The bot listens to every channel").catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    let msg = "```\n"
  ...
  addElem: function(message, prefix, instructions){
    ...
    if(message.content.split(' ').length < 2 || message.mentions.channels.size != 1){
      ...
      message.reply('Incorrect usage of \'' + prefix +  instructions[21][0].name + '\', type \'' + prefix +  instructions[8][0].name + '\' for the correct syntax.').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(!channel) {
      ...
      message.reply('The channel could not be found.').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(ids.includes(channel.id)){
      ...
      message.reply('This channel is already whitelisted').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }

    channels.push({"name":channel.name,"id":channel.id});
    serverManager.setWhitelist(id, channels);
    message.reply("The channel was successfully added.").catch(err => {logManager.writeErrorLog(err.stack);});
  },
  ...
  removeElem: function(message, prefix, instructions){
    ...
    if(message.content.split(' ').length < 2 || message.mentions.channels.size != 1){
      ...
      message.reply('Incorrect usage of \'' + prefix +  instructions[26][0].name + '\', type \'' + prefix +  instructions[8][0].name + '\' for the correct syntax.').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    if(!ids.includes(channel.id)){
      ...
      message.reply('This channel is not whitelisted.').catch(err => {logManager.writeErrorLog(err.stack);});
      return;
    }
    ...
    serverManager.setWhitelist(id, channels);
    message.reply("The channel was successfully removed").catch(err => {logManager.writeErrorLog(err.stack);});
  },

  clear: function(message){
    serverManager.setWhitelist(message.guild.id, []);
    message.reply("The whitelist was successfully reseted.").catch(err => {logManager.writeErrorLog(err.stack);});
  },
