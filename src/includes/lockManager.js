...
function lockChannel (message){
  ...
  channel.setName("[DO NOT DISTURB] " + channel.name).catch();
}
...
function unlockChannel(voiceChannel){
  ...
  if(!voiceChannel.name.startsWith("[DO NOT DISTURB] ")) return;
  voiceChannel.setName(voiceChannel.name.substring(17,voiceChannel.name.length)).catch();
}
